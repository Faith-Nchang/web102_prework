...
const moneyPledged = GAMES_JSON.reduce((total, game) => total + game.pledged, 0);
raisedCard.innerHTML=`$${moneyPledged.toLocaleString()}`
...
const description = `
  A total of $${totalAmountRaised.toLocaleString()} has been raised across ${totalGames} ${totalGames === 1 ? 'game' : 'games'}.
  ${unfundedGamesCount === 0 
    ? 'All games are fully funded!' 
    : `${unfundedGamesCount} ${unfundedGamesCount === 1 ? 'game remains' : 'games remain'} unfunded. We need your help to fund these games!`
  }
`;
...
paragraph.classList.add('fundedData');
